...
export const getCelebs = async (req: Request, res: Response): Promise<void> => {
  try {
    const { category, search } = req.query as Record<
      string,
      string | undefined
    >;
...
export const adminGetCelebs = async (
  _req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const adminGetCeleb = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const createCeleb = async (
  req: Request,
  res: Response,
): Promise<void> => {
  ...
  try {
    const file = req.file as
      ...
      | undefined;
    if (!file) {
      res.status(400).json({ message: "Celebrity image is required." });
      return;
    }

    const price = parsePrice(req.body as Record<string, unknown>);
    const eventEnabled =
      req.body["eventEnabled"] === "true" || req.body["eventEnabled"] === true;
    if (!price.meetup || !price.fancard) {
      res
        .status(400)
        .json({ message: "Meetup and Fan Card prices are required." });
      return;
    }
    if (eventEnabled && !price.event) {
      res
        .status(400)
        .json({ message: "Event price is required when event is enabled." });
      return;
    }
...
export const updateCeleb = async (
  req: Request,
  res: Response,
): Promise<void> => {
  ...
  try {
    ...
    if (file && celeb.imagePublicId) {
      await cloudinary.uploader.destroy(celeb.imagePublicId).catch(() => {
        /* ignore */
      });
    }
...
export const deleteCeleb = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
