...
const parsePrice = (
  ...
  return {
    meetup:  Number(b['price[meetup]']  ?? b.price?.['meetup']  ?? fallback?.meetup  ?? 0),
    event:   Number(b['price[event]']   ?? b.price?.['event']   ?? fallback?.event   ?? 0),
    fancard: Number(b['price[fancard]'] ?? b.price?.['fancard'] ?? fallback?.fancard ?? 0),
  }
}
...
const parseUpcomingDates = (raw: string | undefined): { date: string; location: string }[] => {
  if (!raw || raw.trim() === '') return []
  try {
    const parsed = JSON.parse(raw)
    if (!Array.isArray(parsed)) return []
    return parsed
      .filter((item) => item && typeof item.date === 'string' && typeof item.location === 'string')
      .map((item) => ({ date: item.date.trim(), location: item.location.trim() }))
